...
export default function ReportsPage() {
  ...
  const leadsMes = clientes.filter(c => {
    const fecha = new Date(c.created_at)
    return c.etapa === 'Lead' || fecha >= inicioMes
  }).length
  ...
  // Tiempo promedio de respuesta a leads (desde creación hasta primer contacto)
  const tiemposRespuesta = clientes
    ...
    .map(c => {
      ...
      return (contactado - creado) / (1000 * 60 * 60) // horas
    })
  ...
  const etapaColors: Record<string, string> = {
    'Lead': 'bg-zinc-600', 'Buscando': 'bg-blue-600',
    'En Oferta': 'bg-amber-500', 'Cierre': 'bg-green-500'
  }
  const etapaDots: Record<string, string> = {
    'Lead': 'bg-zinc-400', 'Buscando': 'bg-blue-400',
    'En Oferta': 'bg-amber-400', 'Cierre': 'bg-green-400'
  }
  ...
  const propsPorSector = sectores.map((s, i) => ({
    sector: s,
    total: properties.filter(p => (p.sector || '').toLowerCase() === s.toLowerCase()).length,
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            { label: 'Propiedades', value: properties.length, color: 'text-blue-400', sub: `${properties.filter(p => p.estado === 'disponible').length} disponibles` },
                ...
                {sectorTop.map(([sector, count], i) => (
                  <div key={sector}>
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className="text-amber-400 text-xs font-black ml-2">{count}</span>
                    </div>
                ...
                {clientesPorEtapa.filter(e => e.total > 0).map(e => (
                  <div key={e.etapa} className="flex items-center gap-1.5">
                    ...
                    <span className="text-white text-xs font-bold">
                      {clientes.length > 0 ? Math.round((e.total / clientes.length) * 100) : 0}%
                    </span>
                  </div>
                ))}
              </div>
                  ...
                  <div key={s.sector}>
                    ...
                    <div className="h-2.5 bg-zinc-800 rounded-full overflow-hidden">
                      <div className={`h-full rounded-full transition-all duration-700 ${s.color}`}
                        style={{ width: `${(s.total / maxProps) * 100}%` }} />
                    </div>
                  </div>
                ))}
              </div>
