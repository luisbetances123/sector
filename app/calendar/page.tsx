          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6">
            ...
            <div className="grid grid-cols-7 gap-1">
              {celdas.map((dia, i) => {
                ...
                return (
                  <button key={i} onClick={() => setDiaSeleccionado(fechaStr)} className={`relative p-2 rounded-xl text-sm font-bold transition-all min-h-[52px] flex flex-col items-center ${esSel ? 'bg-amber-500 text-black' : esHoy ? 'bg-zinc-700 text-white' : 'text-zinc-400 hover:bg-zinc-800'}`}>
                    <span>{dia}</span>
                    {eventos.length > 0 && <div className="flex gap-0.5 mt-1 flex-wrap justify-center">{eventos.slice(0,3).map((e,ei) => <div key={ei} className={`w-1.5 h-1.5 rounded-full ${esSel ? 'bg-black' : e.urgencia === 'alta' ? 'bg-red-400' : e.urgencia === 'media' ? 'bg-amber-400' : 'bg-green-400'}`} />)}</div>}
                  </button>
                )
              })}
            </div>
