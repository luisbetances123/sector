    ...
    <nav className="md:hidden fixed bottom-0 left-0 w-full bg-[#050505]/95 backdrop-blur-xl border-t border-white/10 z-[100]">
      <div className="flex justify-around items-center px-1 py-2">
        {menuItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} className={`flex flex-col items-center gap-1 px-2 py-1.5 rounded-xl transition-all ${isActive ? 'bg-white/5' : ''}`}>
              <svg className={`w-5 h-5 ${isActive ? 'text-amber-500' : 'text-gray-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d={item.icon} />
              </svg>
              <span className={`text-[9px] uppercase tracking-tight font-bold ${isActive ? 'text-amber-500' : 'text-gray-600'}`}>{item.name}</span>
            </Link>
          )
        })}
      </div>
