        ...
        <div className="flex justify-around items-center px-1 py-2">
          {menuItems.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} className={`flex flex-col items-center gap-1 px-2 py-1.5 rounded-xl transition-all ${isActive ? 'text-amber-500' : 'text-gray-500'}`}>
                ...
                <span className="text-[11px] uppercase tracking-tight font-bold">{item.name}</span>
              </Link>
            )
          })}
          <button onClick={() => setShowPanel(true)} className="flex flex-col items-center gap-1 px-2 py-1.5 rounded-xl transition-all text-gray-500">
            <div className="relative">
              ...
              {notifs.length > 0 && (
                <span className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-red-500 text-white text-[11px] font-black rounded-full flex items-center justify-center">
                  {notifs.length > 9 ? '9+' : notifs.length}
                </span>
              )}
            </div>
            <span className="text-[11px] uppercase tracking-tight font-bold">Alertas</span>
          </button>
        </div>
      </nav>
    </>
