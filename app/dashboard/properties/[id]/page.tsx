...
export default function PropiedadDetalle() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#09090b] flex items-center justify-center">
      <p className="text-white font-mono animate-pulse">Cargando propiedad...</p>
    </div>
  )
    ...
    <div className="min-h-screen bg-[#09090b] text-zinc-100">
      ...
      <div className="border-b border-zinc-900 px-6 py-4 flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-white font-mono">Portal de Propiedades</p>
        </div>
        <div className="flex gap-3">
          <button onClick={() => router.push("/listings")} className="text-white border border-zinc-800 px-4 py-2 rounded-xl text-xs font-medium hover:text-white transition-colors">
            ← Volver
          </button>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2 space-y-6">
            <div>
              <p className="text-xs font-mono text-white uppercase tracking-wider">{propiedad.location}</p>
            ...
            <div className="grid grid-cols-3 gap-4">
              {propiedad.bedrooms && (
                <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-4 text-center">
                  ...
                  <p className="text-[10px] text-white uppercase tracking-wider mt-1">Recámaras</p>
                </div>
              )}
              {propiedad.bathrooms && (
                <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-4 text-center">
                  <p className="text-2xl font-black text-white font-mono">{propiedad.bathrooms}</p>
                  <p className="text-[10px] text-white uppercase tracking-wider mt-1">Baños</p>
                </div>
              )}
              {propiedad.m2 && (
                <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-4 text-center">
                  <p className="text-2xl font-black text-white font-mono">{propiedad.m2}</p>
                  <p className="text-[10px] text-white uppercase tracking-wider mt-1">m²</p>
                </div>
              )}
            </div>
            ...
            {propiedad.descripcion && (
              <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-5">
                <h3 className="text-xs font-mono text-white uppercase tracking-wider mb-3">Descripción</h3>
                <p className="text-white text-sm leading-relaxed">{propiedad.descripcion}</p>
              </div>
            )}
            ...
            {mapUrl && (
              <div className="bg-zinc-950 border border-zinc-800 rounded-xl overflow-hidden">
                <div className="px-5 py-3 border-b border-zinc-800">
                  <h3 className="text-xs font-mono text-white uppercase tracking-wider">Ubicación</h3>
                  <p className="text-white text-sm mt-0.5">{propiedad.location}, Santo Domingo, RD</p>
                </div>
                <iframe
                  src={mapUrl}
                  width="100%"
          ...
          <div>
            <div className="bg-zinc-950 border border-zinc-800 rounded-2xl p-6 sticky top-6 space-y-4">
              <div>
                ...
                <p className="text-white text-xs mt-1">Un agente te contactará pronto.</p>
              </div>
              {enviado ? (
                ...
                <div className="space-y-3">
                  ...
                  <a href={`https://wa.me/19293056500?text=${encodeURIComponent('Hola, me interesa: ' + propiedad.title)}`}
                    target="_blank"
                    className="w-full flex items-center justify-center gap-2 bg-zinc-900 border border-zinc-800 text-white text-xs rounded-xl py-3 hover:border-[#CCFF00] hover:text-white transition-colors font-bold">
