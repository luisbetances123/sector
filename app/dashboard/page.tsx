...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] p-4 md:p-8 pb-28">
        ...
        <div className="flex gap-3 overflow-x-auto pb-1 mb-8 scrollbar-hide">
          {[
            ...
            <Link key={s.label} href={s.href}
              className="shrink-0 bg-zinc-800/60 border border-zinc-700 rounded-2xl px-5 py-3 hover:border-amber-500/50 transition-all">
          ...
          <div className={`rounded-3xl p-6 border-2 ${leads.length > 0 ? 'bg-red-950/80 border-red-700/60' : 'bg-zinc-800/40 border-zinc-700/40'}`}>
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-3">
                <div className={`w-3 h-3 rounded-full ${leads.length > 0 ? 'bg-red-500 animate-pulse' : 'bg-zinc-600'}`} />
                <span className={`font-black text-base uppercase tracking-wider ${leads.length > 0 ? 'text-red-300' : 'text-zinc-400'}`}>
                  Leads sin responder
                </span>
                <span className={`text-sm font-black px-3 py-1 rounded-full ${leads.length > 0 ? 'bg-red-600 text-white' : 'bg-zinc-700 text-zinc-400'}`}>
                  {leads.length}
                </span>
              </div>
            ...
            {leads.length === 0 ? (
              ...
              <div className="flex flex-col gap-2">
                {leads.slice(0, 5).map(c => (
                  <div key={c.id} className="flex items-center justify-between bg-black/30 border border-red-900/40 rounded-2xl px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-zinc-500 text-xs">{c.telefono || c.email || '—'}</p>
                      </div>
                    </div>
                    <div className="flex gap-2">
                      ...
                      {c.telefono && (
                        <a href={`tel:${c.telefono}`}
                          className="bg-zinc-700 hover:bg-zinc-600 text-white px-3 py-1.5 rounded-xl text-xs font-black transition-colors">
                          📞
                        </a>
                      )}
                    </div>
          ...
          <div className="bg-zinc-800/50 border border-zinc-700/50 rounded-3xl p-6">
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-3">
                ...
                <span className="text-white font-black text-base uppercase tracking-wider">Agenda de hoy</span>
              ...
              <Link href="/hoy" className="text-zinc-500 hover:text-amber-400 text-xs uppercase tracking-wider transition-colors">
                Ver agenda →
              </Link>
            </div>
            {followupsHoy.length === 0 ? (
              ...
              <div className="flex flex-col gap-2">
                {followupsHoy.map(f => (
                  <div key={f.id} className="flex items-center gap-4 bg-zinc-900/60 border border-zinc-700 rounded-2xl px-4 py-3">
                    <span className="text-2xl shrink-0">{tipoIcono[f.tipo] || '📌'}</span>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div className="bg-zinc-800/50 border border-zinc-700/50 rounded-3xl p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <Link href="/pipeline" className="text-zinc-500 hover:text-amber-400 text-xs uppercase transition-colors">Ver →</Link>
            </div>
            {clientesActivos.length === 0 ? (
              ...
              <div className="flex flex-col gap-2">
                {clientesActivos.slice(0, 4).map(c => (
                  <div key={c.id} className="flex items-center justify-between bg-zinc-900/60 rounded-2xl px-3 py-2.5">
                    ...
                    {c.telefono && (
                      <a href={`https://wa.me/${c.telefono.replace(/\D/g, '')}`} target="_blank" rel="noreferrer"
                        className="bg-green-700 hover:bg-green-600 text-white px-2.5 py-1.5 rounded-xl text-[10px] font-black transition-colors">
                        💬
                      </a>
                    )}
                  </div>
        ...
        <section className="mb-6">
          <div className="bg-zinc-800/50 border border-zinc-700/50 rounded-3xl p-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <Link href="/properties" className="text-zinc-500 hover:text-amber-400 text-xs uppercase tracking-wider transition-colors">
                Ver todas →
              </Link>
            </div>
            <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2">
              {SECTORES.map(s => (
                <Link key={s} href={`/properties?sector=${encodeURIComponent(s)}`}
                  className="bg-zinc-700/50 hover:bg-amber-500 hover:text-black text-zinc-300 text-center py-2.5 px-2 rounded-xl text-xs font-black uppercase tracking-wide transition-all">
                  {s}
                </Link>
              ))}
            </div>
