  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans">
      <nav className="flex justify-between items-center p-6 max-w-7xl mx-auto border-b border-white/5">
        ...
        <div className="flex items-center gap-4 text-[10px] text-gray-500 tracking-widest uppercase font-bold">
          Luis Betances
          <div className="w-8 h-8 bg-[#d4af37] rounded-full flex items-center justify-center text-black text-xs font-bold ml-2">LB</div>
        </div>
      </nav>
      ...
      <main className="max-w-7xl mx-auto p-6 md:p-12">
        <header className="mb-12">
          ...
          <p className="text-gray-500 text-sm mt-2 font-light italic">Gestiona tu cartera de clientes con precisión.</p>
        </header>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-white/5">
                {tableData.map((row) => (
                  <tr key={row.name} className="hover:bg-white/[0.02] transition-colors cursor-default">
                    <td className="px-8 py-6 font-light">{row.name}</td>
                    <td className="px-8 py-6 text-gray-500 text-sm italic font-light">{row.prop}</td>
